...
export default function Favorites() {
  ...
  return (
    <div className="h-full flex flex-col bg-transparent animate-in fade-in slide-in-from-right-4 duration-300 relative">
      <div className="love-card px-5 pt-8 pb-3 z-10 shadow-sm relative flex items-center justify-between">
      ...
      {isFavSearchVisible && (
        <div className="love-card px-5 py-3 border-b border-transparent animate-in slide-in-from-top-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 relative">
        <ScrollableRow className="flex space-x-2 mb-4 pb-1">
          <span onClick={() => setFavFilter('全部')} className={`text-[11px] px-4 py-1.5 rounded-full whitespace-nowrap cursor-pointer transition-colors shadow-sm flex items-center ${favFilter === '全部' ? 'bg-gray-800 text-white font-bold' : 'love-card text-gray-600 border border-transparent'}`}>全部</span>
          {customFolders.map(folder => (
            <span key={folder} onClick={() => setFavFilter(folder)} className={`text-[11px] px-4 py-1.5 rounded-full whitespace-nowrap cursor-pointer transition-colors shadow-sm flex items-center ${favFilter === folder ? 'bg-pink-500 text-white font-bold' : 'love-card text-gray-600 border border-transparent'}`}>
              <FolderHeart size={12} className="mr-1"/> {folder}
            </span>
          ))}
          <span onClick={() => setIsCreateFolderModalOpen(true)} className="text-[11px] px-3 py-1.5 rounded-full whitespace-nowrap cursor-pointer transition-colors bg-transparent text-gray-500 border border-dashed border-gray-300 flex items-center"><PlusCircle size={12} className="mr-1"/> 新建</span>
        </ScrollableRow>
        ...
        <div className="flex justify-between items-center mb-3 px-1">
          ...
          <div className="flex items-center space-x-2">
            <div className="relative">
               <span onClick={() => setIsSortDropdownOpen(!isSortDropdownOpen)} className="text-[11px] love-card shadow-sm border border-transparent px-3 py-1 rounded-full cursor-pointer text-gray-600 hover:bg-transparent flex items-center">
               ...
               {isSortDropdownOpen && (
                 <>
                   ...
                   <div className="absolute right-0 top-full mt-1 w-24 love-card rounded-lg shadow-lg border border-transparent py-1 z-20 animate-in fade-in zoom-in-95">
            ...
            {myFavs.length > 0 && (
              <span onClick={() => { setIsEditFav(!isEditFav); setCheckedFavs([]); setIsSortDropdownOpen(false); }} className={`text-[11px] love-card shadow-sm border px-3 py-1 rounded-full cursor-pointer transition-colors ${isEditFav ? 'border-pink-500 text-pink-500 bg-pink-50' : 'border-transparent text-gray-600 hover:bg-transparent'}`}>
                {isEditFav ? '取消管理' : '批量管理'}
              </span>
            )}
          </div>
        ...
        {myFavs.length > 0 ? (
          <div className="space-y-4 animate-fade-in-up">
            {myFavs.map(script => (
              <div key={script.id} className="relative flex items-start">
                ...
                <div className={`flex-1 transition-all ${isEditFav ? 'opacity-80 pointer-events-none' : ''}`}>
                  ...
                  <div className="love-card/80 backdrop-blur-sm border-t border-transparent rounded-b-[1rem] px-3 py-2 -mt-3 pt-4 flex justify-between items-center relative z-0 shadow-sm">
                     ...
                     <div className="flex items-center space-x-2 shrink-0">
                       <button onClick={() => setMovingScriptId(script.id)} className="text-[10px] text-gray-500 hover:text-blue-500 flex items-center bg-transparent px-2 py-1 rounded transition-colors">
           ...
           <div className="flex flex-col items-center justify-center py-20 opacity-60">
              ...
              <button className="mt-4 love-card border border-gray-200 text-gray-600 text-xs px-4 py-2 rounded-full shadow-sm" onClick={() => { setFavSearchQuery(''); setFavFilter('全部'); }}>清除条件</button>
           </div>
        )}
      </div>

      {isEditFav && (
        <div className="absolute bottom-20 w-full love-card border-t border-transparent p-3 px-5 z-40 flex justify-between items-center animate-in slide-in-from-bottom-5">
      ...
      {isCreateFolderModalOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center">
           ...
           <div className="love-card w-[80%] rounded-2xl p-5 relative z-10 animate-in zoom-in-95 duration-200 shadow-xl">
              ...
              <input
                ...
                className="w-full bg-transparent border border-gray-200 rounded-xl p-3 text-[13px] outline-none focus:border-pink-300 focus:love-card transition-all shadow-inner text-center"
              />
      ...
      {movingScriptId !== null && (
        <div className="absolute inset-x-0 bottom-0 z-50">
          ...
          <div className="love-card rounded-t-2xl p-4 pb-8 relative z-10 animate-in slide-in-from-bottom-full duration-300 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
             ...
             <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl text-sm flex items-center justify-between transition-colors ${!scriptFolders[movingScriptId] ? 'bg-pink-50 text-pink-600 font-bold' : 'bg-transparent text-gray-700 hover:bg-gray-100'}`}
                >
                ...
                {customFolders.map(folder => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl text-sm flex items-center justify-between transition-colors ${scriptFolders[movingScriptId] === folder ? 'bg-pink-50 text-pink-600 font-bold' : 'bg-transparent text-gray-700 hover:bg-gray-100'}`}
                  >
      ...
      {editingNoteId !== null && (
        <div className="absolute inset-0 z-50 flex items-center justify-center">
           ...
           <div className="love-card w-[85%] rounded-[1.5rem] p-5 relative z-10 animate-in zoom-in-95 duration-200 shadow-xl">
              ...
              <textarea
                ...
                className="w-full bg-transparent border border-gray-200 rounded-xl p-3 text-[13px] outline-none focus:border-pink-300 focus:love-card transition-all resize-none shadow-inner"
                rows={4}
              />
              <div className="flex space-x-3 mt-4">

    ...
    <div className="h-full overflow-y-auto pb-24 scrollbar-hide bg-[#F5F7FA]">
      ...
      <div className="px-5 mt-5 grid grid-cols-4 gap-3 animate-fade-in-up delay-100">
        {[
          ...
          <div key={idx} onClick={() => { setActiveTab('discover'); setDiscoverState(prev => ({ ...prev, activeCategory: item.catId, searchQuery: '' })); }} className="flex flex-col items-center justify-center bg-white p-3 rounded-2xl shadow-sm cursor-pointer active:scale-95 transition-transform">
              ...
              <div key={course.id} onClick={() => course.link ? window.open(course.link, '_blank') : setActiveServicePage({ id: 'course', data: course })} className="relative min-w-[240px] h-[160px] rounded-[1.5rem] overflow-hidden shadow-lg shadow-pink-200/40 group bg-gradient-to-br from-rose-400 via-pink-400 to-purple-500 cursor-pointer flex-shrink-0 ring-2 ring-pink-100/50 hover:ring-pink-300 transition-all duration-300">
                ...
                <div className="absolute inset-0 p-4 flex flex-col justify-end">
                  <div className="flex items-center space-x-2 mb-1.5">
                    ...
                    {course.rating > 0 && (
                      <span className="bg-white/20 backdrop-blur-md text-white text-[10px] px-2 py-0.5 rounded-md flex items-center border border-white/10">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button className="flex items-center text-[10px] font-bold text-gray-900 bg-white rounded-full px-2.5 py-1 shadow-sm group-hover:bg-pink-50 transition-colors">
      ...
      <div className="px-4 mt-3 animate-fade-in-up delay-[320ms]">
        <div className="bg-gradient-to-br from-white to-pink-50/50 border border-pink-100 rounded-[1.5rem] p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group hover:shadow-[0_8px_30px_rgb(236,72,153,0.1)] transition-all duration-500">
           ...
           <p className="text-[13px] text-gray-700 mb-4 bg-white/80 backdrop-blur-sm p-3.5 rounded-xl border border-pink-100/50 relative z-10 shadow-sm leading-relaxed">女生突然对你说："我好像发烧到39度了，好难受..." 你该怎么回？</p>
           <div className="space-y-2.5 relative z-10">
             <button onClick={()=>showToast('太直男了！扣10分！正确做法请看解析')} className="w-full bg-white/90 hover:bg-white hover:text-rose-500 hover:border-rose-200 border border-white p-3.5 rounded-xl text-left text-[13px] text-gray-600 transition-all flex justify-between items-center shadow-sm hover:shadow-md active:scale-[0.98]">
               <span>A. "多喝热水，吃药了吗？快去休息吧。"</span>
             </button>
             <button onClick={()=>showToast('高情商！加10分！')} className="w-full bg-white/90 hover:bg-white hover:text-green-600 hover:border-green-200 border border-white p-3.5 rounded-xl text-left text-[13px] text-gray-600 transition-all flex justify-between items-center shadow-sm hover:shadow-md active:scale-[0.98]">
               <span>B. "开门，我在楼下带了药和粥。"</span>
             </button>
           </div>
           <div className="mt-4 pt-3 border-t border-pink-200/50 text-center relative z-10">
             <span className="text-rose-500 text-[11px] font-bold cursor-pointer hover:text-rose-600 transition-colors flex items-center justify-center bg-white/50 w-max mx-auto px-4 py-1.5 rounded-full" onClick={()=> {setActiveTab('ai'); setAiState({ chatInput: '女生说她发烧到39度了，除了说多喝热水，我该怎么高情商回复并表现出关心？' });}}>

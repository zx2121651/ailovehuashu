  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#F8F9FA] rounded-t-3xl z-[70] animate-in slide-in-from-bottom-full duration-300 pb-safe">
        <div className="love-card rounded-t-3xl p-5 shadow-sm relative border-b border-transparent">
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto scrollbar-hide love-card">
           <div>
              ...
              <textarea rows={2} className="w-full bg-transparent border border-gray-200 rounded-xl p-3.5 text-[14px] focus:border-gray-800 focus:love-card transition-all outline-none resize-none placeholder-gray-400" placeholder="例如：你到底喜欢我什么？"></textarea>
           </div>
           ...
           <div>
              ...
              <textarea rows={3} className="w-full bg-transparent border border-gray-200 rounded-xl p-3.5 text-[14px] focus:border-gray-800 focus:love-card transition-all outline-none resize-none placeholder-gray-400" placeholder="例如：喜欢你这个问题问得好，让我有了一辈子时间来回答。"></textarea>
           </div>
              ...
              <div className="flex flex-wrap gap-2.5">
                 {categories.map(c => (
                   <span
                     ...
                     className={`text-[12px] px-4 py-2 border rounded-full cursor-pointer transition-all shadow-sm ${contributeCategory === c.id ? 'bg-gray-900 text-white border-gray-900 font-bold scale-105' : 'love-card text-gray-600 border-gray-200 hover:bg-transparent'}`}
                   >
                     {c.name}
                   </span>
                 ))}
